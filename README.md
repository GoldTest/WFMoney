...
EasyMoney AI 是一款基于人工智能的金融行情分析与交易预演系统。它集成了实时数据获取、AI 智能策略分析以及回测预演功能，旨在帮助用户通过 AI 辅助决策提升投资效率。
...
所有持久化数据（如 AI 配置、持仓记录等）均存储在本地 Windows 系统的 `%LOCALAPPDATA%\EasyMoneyAI` 目录下，不会上传到除您配置的 AI API 以外的任何第三方服务器。
